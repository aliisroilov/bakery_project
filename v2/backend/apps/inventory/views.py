...
class IngredientViewSet(viewsets.ModelViewSet):
    ...
    def adjust_stock(self, request, pk=None):
        ...
        with transaction.atomic():
            ...
            # Snapshot the discrepancy value into Tan narxi (COGS): +shortage / −surplus.
            value = ((old_qty - new_qty) * (locked.avg_cost_uzs or 0)).quantize(Decimal("0.01"))
...
class InventoryRevisionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def batch_revision(self, request):
        ...
        A revision is a NON-CASH inventory adjustment: each item's discrepancy value
        ((old − new) × avg_cost) is snapshotted onto the row (value_uzs) and folds into
        Tan narxi (COGS) in the reports — a shortage (kamomad) raises cost of goods, a
        surplus (ortiqcha) lowers it. The kassa is never touched.
        """
        ...
        with transaction.atomic():
            for row in items_data:
                ...
                old_qty = ing.quantity
                # Snapshot the discrepancy value BEFORE mutating stock: +shortage
                # (kamomad, adds to COGS) / −surplus (ortiqcha, reduces COGS).
